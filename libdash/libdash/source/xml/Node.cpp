...
dash::mpd::SegmentBase*                     Node::ToSegmentBase         ()  const
{
    ...
    {
		//2015.4.5 - php
        if (subNodes.at(i)->GetName() != "Initialization" && subNodes.at(i)->GetName() != "Initialisation" && subNodes.at(i)->GetName() != "RepresentationIndex")
...
dash::mpd::SegmentList*                     Node::ToSegmentList         ()  const
{
    ...
    for(size_t i = 0; i < subNodes.size(); i++)
    {
        ...
        if (subNodes.at(i)->GetName() != "SegmentTimeline" && subNodes.at(i)->GetName() != "BitstreamSwitching" &&
            subNodes.at(i)->GetName() != "Initialization" && subNodes.at(i)->GetName() != "Initialisation"  && subNodes.at(i)->GetName() != "RepresentationIndex")
...
dash::mpd::SegmentTemplate*                 Node::ToSegmentTemplate     ()  const
{
	...
	//2015.4.5 - php
	if (this->HasAttribute("initialisation"))
	{
		segmentTemplate->SetInitialization(this->GetAttributeValue("initialisation"));
	}
