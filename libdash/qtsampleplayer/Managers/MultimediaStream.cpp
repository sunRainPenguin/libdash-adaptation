...
void        MultimediaStream::Init                      ()
{
    ...
    this->frameBuffer   = new Buffer<VideoFrame>(this->frameBufferSize, libdash::framework::buffer::VIDEO);			//2015.4.8 - php
    this->sampleBuffer  = new Buffer<AudioSamples>(this->sampleBufferSize, libdash::framework::buffer::AUDIO);		//2015.4.18 - php
...
void        MultimediaStream::AddFrame                  (VideoFrame *frame)
{
    this->frameBuffer->PushBack(frame);
}
VideoFrame*     MultimediaStream::GetFrame                  ()
{
    return this->frameBuffer->GetFront();				//2015.4.10 - php
}

void        MultimediaStream::AddSamples                (AudioSamples *samples)
{
    this->sampleBuffer->PushBack(samples);
}

AudioSamples* MultimediaStream::GetSamples                ()
{
