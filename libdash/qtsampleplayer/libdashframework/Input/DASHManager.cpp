...
void        DASHManager::OnVideoFrameDecoded    (const uint8_t **data, videoFrameProperties* props)
{
	...
	{
		if (data == NULL && props->isLastFrame)
		{
			VideoFrame*   frame = new VideoFrame(NULL, props->segmentIndex, props->isLastFrame);			//2015.4.8 - php
			this->multimediaStream->AddFrame(frame);
			return;
		}
		else
		{
	...
	VideoFrame*   frame = new VideoFrame(image, props->segmentIndex, props->isLastFrame);			//2015.4.8 - php
    this->multimediaStream->AddFrame(frame);
...
void        DASHManager::OnAudioSampleDecoded   (const uint8_t **data, audioFrameProperties* props)
{
	...
	AudioSamples *samplesWithTags = new AudioSamples(samples, props->segmentIndex, props->isLastFrame);

    this->multimediaStream->AddSamples(samplesWithTags);
}
