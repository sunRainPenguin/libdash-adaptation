...
DASHReceiver::~DASHReceiver     ()
{
	...
	//2015.4.23 - php
	for (std::map<dash::mpd::IRepresentation*, MediaObject*>::iterator i=initSegments.begin(); i!=initSegments.end();)  
	{  
		delete(i->second);
		i->second=NULL;
		initSegments.erase(i++);
	}  
	//delete this->currDownloadingMO;
...
MediaObject*                DASHReceiver::GetNextSegment            ()
{
    ...
    if (seg != NULL)
    {
        MediaObject *media = new MediaObject(seg, this->representation, this->segmentNumber + this->segmentOffset, this);			//2015.4.7 - php
        this->segmentNumber++;
        return media;
    }
...
MediaObject*                DASHReceiver::GetSegment                (uint32_t segNum)
{
    ...
    if (seg != NULL)
    {
        MediaObject *media = new MediaObject(seg, this->representation, segNum + segmentOffset, this);			//2015.4.7 - php
        return media;
    }
...
MediaObject*                DASHReceiver::GetInitSegment            ()
{
    ...
    if (seg != NULL)
    {
        MediaObject *media = new MediaObject(seg, this->representation, 0, this);			//2015.4.7 - php
        return media;
    }
